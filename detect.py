...
if __name__ == "__main__":
    ...
    batches = list(map(prep_image, imlist, [inp_dim for _ in range(len(imlist))]))
    ...
    for batch in im_batches:
        ...
        if write:
            output = torch.cat((prediction, output))
    ...
    def write(x, batches, results):
        c1 = x[1:3].numpy().astype(int)
        c2 = x[3:5].numpy().astype(int)
        img = results[int(x[0])]
        cls = int(x[-1])
